  ...
  // adjust time interval between 1 - 59 mins for session
    $('#minus-session').click(function(){
      if(standardSessionMins > 1){
        ...
        $('#set-session').text(("0" + standardSessionMins).slice(-2) + ":00");
        $('#current-time').text(("0" + standardSessionMins).slice(-2) + "m" + ":00" + "sec");
      }
    });
  ...
  $('#plus-break').click(function(){
    ...
    function startTimer(standardSessionMins) {
      ...
      function myTimer() {
          ...
          document.getElementById("current-time").innerHTML = ("0" + minutes).slice(-2) + "m " + ("0" + seconds).slice(-2) + "s";
          ...
          if (interval < 0) {
            // clear the timer set with the setInterval() method
              clearInterval(x);
              document.getElementById("current-time").innerHTML = "Time Up Take a Break";
              window.setTimeout(function(){startBreak(breakTime)},2000);
    ...
    function progressBar(setTime, barColor) {
      ...
      var width = 1;
      var intervalId = setInterval(frame, setTime);
      function frame() {
        if (width >= 100) {
